...
class SharedMemoryManager:
    def __init__(self):
        ...
        self._map_shm()
        self.sem_cwrite = posix_ipc.Semaphore("/cwrite_mutex")
        self.sem_cread = posix_ipc.Semaphore("/cread_mutex")
    ...
    def _wait_for_resources(self):
        ...
        while True:
            try:
                # 检测共享内存文件是否存在
                os.stat("/dev/shm/cwrite_shm")
                os.stat("/dev/shm/cread_shm")
                return
    ...
    def _map_shm(self):
        # 打开共享内存文件描述符
        fd_cwrite = os.open("/dev/shm/cwrite_shm", os.O_RDWR)
        fd_cread = os.open("/dev/shm/cread_shm", os.O_RDWR)
        
        # 内存映射
        self.mmap_cwrite = mmap.mmap(fd_cwrite, ctypes.sizeof(CWriteStruct))
        self.mmap_cread = mmap.mmap(fd_cread, ctypes.sizeof(CReadStruct))
        
        # 转换为结构体指针
        self.cwrite = CWriteStruct.from_buffer(self.mmap_cwrite)
        self.cread = CReadStruct.from_buffer(self.mmap_cread)

    def read_cwrite(self):
        """ 从C端写入区域读取数据 """
        self.sem_cwrite.acquire()
        try:
            if self.cwrite.has_new:
                # 提取结构体数据
                data = {
                    "state": list(self.cwrite.states_reward.state),
                    "reward": self.cwrite.states_reward.reward
                }
                self.cwrite.has_new = 0
                return data
            return None
        finally:
            self.sem_cwrite.release()

    def write_cread(self, action: float):
        """ 向C端读取区域写入动作 """
        self.sem_cread.acquire()
        try:
            self.cread.action = action
            self.cread.has_new = 1
        finally:
            self.sem_cread.release()

    def cleanup(self):
        """ 资源清理 """
        self.mmap_cwrite.close()
        self.mmap_cread.close()
        self.sem_cwrite.close()
        self.sem_cread.close()
    ...
    try:
        ...
        while True:
            ...
            if has_new_data and latest_state:
                ...
                log_writer.writerow([f'action:{action}|', f'cwnd:{cwnd}|',f"throughput:{throughput}|", f"max_throughput:{max_throughput}|",f"delay:{delay}us|",f"min_delay:{min_delay}us|", f"minRtt:{minRtt}us|",f"ackgap:{ackgap}|", f"latest_reward:{latest_reward}|"])
                log_file.flush()
                manager.write_cread(action)                
